...
namespace RMAWindowsService
{
    class ADSecurity
    {
        ...
        private static readonly LogWriter _log = HostLogger.Get<ADSecurity>();
        ...
        {
            _log.InfoFormat("ADSecurity Running..");
            ...
            using (var context = new PrincipalContext(ContextType.Domain))
            {
                using (var searcher = new PrincipalSearcher(new UserPrincipal(context)))
                {
                    foreach (var result in searcher.FindAll())
                    {
                        ...
                        foreach (var item in Enum.GetValues(typeof(Enums.Modules)))
                        {
            ...
            var lstModules = Enum.GetNames(typeof(Enums.Modules)).Select(val => new ModuleModel() { Name = val }).ToList();
